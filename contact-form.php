...
//Your E-mail
$your_email = 'caribettech@gmail.com';

$a=$_POST['select'];
settype($a,"string");
//Default Subject if 'subject' field not specified
$default_subject = 'Portal Web CaribeTech'.' - Asunto: ' .$a;

//Message if 'name' field not specified
$name_not_specified = 'Web-Caribetech';

//Message if e-mail sent successfully
$email_was_sent = 'Gracias, Su mensaje se envio correctamente.';
...
if(isset($_POST['name'])) {
	...
	if(empty($errors)) {
		...
		foreach ($_POST as $key => $value) {
			...
			if ($key == 'name' || $key == 'message' || $key == 'subject' || $key == 'email' || $key == 'select'  ) {
				continue;
			}
